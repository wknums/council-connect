...
def _req(method: str, path: str, councillor_id: str, body: dict | None = None, route_params: dict | None = None):
...
def test_campaign_creation_and_metrics(councillor_id):
    ...
    assert add_contact.status_code == 201
    # Create campaign
    camp_resp = function_app.campaigns(_req('POST', '/campaigns', councillor_id, {"subject": "Update", "content": "Hello", "listIds": [list_id]}))
    assert camp_resp.status_code == 201
    camp_id = json.loads(camp_resp.get_body())['id']
    # List campaigns
    list_camps = json.loads(function_app.campaigns(_req('GET', '/campaigns', councillor_id)).get_body())['items']
...
def test_tracking_events_affect_metrics(councillor_id):
    ...
    camp_id = json.loads(function_app.campaigns(_req('POST', '/campaigns', councillor_id, {"subject": "Notice", "content": "Body", "listIds": [list_id]})).get_body())['id']
...
def test_delete_campaign(councillor_id):
    ...
    assert contact_resp.status_code == 201
    camp_resp = function_app.campaigns(_req('POST', '/campaigns', councillor_id, {"subject": "S", "content": "Body", "listIds": [list_id]}))
    assert camp_resp.status_code == 201
    camp_id = json.loads(camp_resp.get_body())['id']
    # Delete
    del_resp = function_app.delete_campaign(_req('DELETE', f'/campaigns/{camp_id}', councillor_id, route_params={'campaignId': camp_id}))
    assert del_resp.status_code == 200
    campaigns = json.loads(function_app.campaigns(_req('GET', '/campaigns', councillor_id)).get_body())['items']
